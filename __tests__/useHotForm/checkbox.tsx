...
const initialSchema: HotFormSchema<CheckboxFormData> = {
  ...
  letters: {
    ...
    validator: (value: Array<string>) =>
      !!value.length && value.every(v => letterValues.includes(v)),
  },
}

const initialSchemaFieldNames = Object.keys(initialSchema) as Array<
  keyof HotFormSchema<CheckboxFormData>
>

interface RenderCheckboxFormResult extends RenderResult {
  current: UseHotFormReturnType<CheckboxFormData>
  submitButton: HTMLButtonElement
  testForm: HTMLFormElement
  activeInput: HTMLInputElement
  lettersInputContainer: HTMLDivElement
}

const renderCheckboxForm = (
  config: HotFormConfig<CheckboxFormData>,
): RenderCheckboxFormResult => {
  ...
  return {
    ...
    lettersInputContainer: renderResult.getByTestId(
      ...
      await Promise.all(
        initialSchemaFieldNames.map(async fieldName => {
          const input = renderResult[
            `${fieldName}Input` as keyof RenderCheckboxFormResult
          ] as HTMLInputElement

          const inputContainer = renderResult[
            `${fieldName}InputContainer` as keyof RenderCheckboxFormResult
          ] as HTMLDivElement
        ...
        Object.entries(nextFieldValues).map(async ([fieldName, value]) => {
          ...
          const currentSchemaField =
            renderResult.current.currentSchema[
              fieldName as keyof CheckboxFormData
            ]
