...
const TextAndNumberForm: React.FC<UseHotFormReturnType<TextAndNumberFormData>> = ({
  currentSchema,
  handleBlur,
  handleChange,
  handleReset,
...
const initialSchemaFieldNames = Object.keys(initialSchema) as Array<keyof HotFormSchema<TextAndNumberFormData>>;

const renderTextAndNumberForm = (config: HotFormConfig<TextAndNumberFormData>) => {
  ...
  return {
    ...
    ticketNumberInput: renderResult.getByTestId('ticket-number-input') as HTMLInputElement,
    usernameInput: renderResult.getByTestId('username-input') as HTMLInputElement
  };
}
...
describe('TextAndNumberForm Component', () => {
  it('MUST initialize the field values', () => {
    ...
    initialSchemaFieldNames.map(fieldName => {
  ...
  describe('Change Event', () => {
    it('MUST update field values based on name attribute', () => {
      ...
      Object.entries(expectedFieldValues).map(([ fieldName, value ]) => {
        const input = renderResult[`${fieldName}Input` as keyof typeof renderResult] as HTMLInputElement;
    ...
    it('SHOULD validate schema field when `hotField` is true (default) if validators exist', async () => {
      ...
      await Promise.all(Object.entries(nextFieldValues).map(async ([ fieldName, value ]) => {
        const input = renderResult[`${fieldName}Input` as keyof typeof renderResult] as HTMLInputElement;
        const currentSchemaField = renderResult.current.currentSchema[fieldName as keyof TextAndNumberFormData];
  ...
  describe('Reset Event', () => {
    ...
    it('SHOULD reset schema when the form is reset', () => {
      ...
      Object.entries(nextFieldValues).map(([ fieldName, value ]) => {
        const input = renderResult[`${fieldName}Input` as keyof typeof renderResult] as HTMLInputElement;
    ...
    it('SHOULD call `onReset` if `onReset` callback is set', async () => {
      ...
      await waitFor(() => expect(onReset).toHaveBeenCalledWith(expectedFieldValues));
    });
  });
  ...
  describe('Submit Event', () => {
    ...
    it('SHOULD NOT call `onInvalid` if `onInvalid` callback is set and the field values are valid', async () => {
      ...
      await waitFor(() => expect(onInvalid).not.toHaveBeenCalled());
    });
